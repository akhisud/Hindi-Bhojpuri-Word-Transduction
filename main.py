...
from keras.layers import Activation, TimeDistributed, Dense, RepeatVector, recurrent, Embedding, Input, merge, GRU
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, hidden_size, num_layers):
	...
	emb_layer = Embedding(X_vocab_len, 512, input_length=X_max_len, mask_zero=True) # DEFINITION of layer
	...
	#ENCODER LSTM:
	LtoR_LSTM = GRU(256, return_sequences=False)
	LtoR_LSTM_vector = LtoR_LSTM(hindi_word_embedding)
	RtoL_LSTM = GRU(256, return_sequences=False, go_backwards=True)
	...
	for _ in range(num_layers):
		LtoR_LSTM = GRU(256, return_sequences=True)
...
if MODE == 'train':
	...
	if(len(saved_weights) != 0):
		...
		history = model.fit(X, y_sequences, validation_split= 0.1, batch_size=BATCH_SIZE, epochs = EPOCHS, 
											verbose=2,callbacks=[EarlyStopping(patience=300,verbose=1), 
...
# performing test by loading saved training weights if we choose test mode
	...
	if len(saved_weights) == 0:
		...
		filename = 'LSTM_attention_result_'+str(LAYER_NUM)+'layers_'+str(BATCH_SIZE)+'batches.txt'
